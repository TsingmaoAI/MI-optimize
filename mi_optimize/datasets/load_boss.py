...
SA_labels = ["negative", "positive", "neutral"]
SA_label_mapping = {"0":"negative", "1":"positive", "2":"neutral"}

TD_labels = ["benign", "toxic"]
TD_label_mapping = {"benign":0, "toxic":1}

NLI_labels = ["Yes", "Maybe", "No"]
NLI_label_mapping = {"Yes":0, "Maybe":1, "No": 2}
...
NER_labels = list(NER_label_mapping.keys())

def get_calibrate_boss(task_name, dataset_name, nsamples=208, split='train', shuffle=False, seed=42):
    """
...
def get_BOSS(task_name, dataset_name, split, shuffle=False, seed=42):
    data_dir = f"datasets/BOSS/{task_name}/{dataset_name}"
    if task_name == "QuestionAnswering":
        examples = []
        with open(f"datasets/BOSS/QuestionAnswering/{dataset_name}/{split}.json",'r') as f:
            lines = f.readlines()
        for line in lines:
        ...
        raise ValueError(f'not support {task_name}')
...
### Example ###\n"
        ...
        # qa_question_list = get_str(task_name, dataset_name,fewshot_num,split,shuffle,seed) #这里get_calibrate的问题模版是跟数据集一模一样，与prompt不一样
        # for content in qa_question_list:
...
### Input ###\n"
        ...
        if dataset_name == "conll" or dataset_name == "ener":
            prompt = f"### Instruction ###\n\
